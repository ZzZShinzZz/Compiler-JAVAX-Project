...
public class ProgramGenerator extends CodeGenerator
{
     ...
     */
//    public void emitProgram(jsParser.ProgramContext ctx)
//    {
//
...
//    }
    public void emitProgram(jsParser.ProgramContext ctx)
    {
        ...
        emitInputScanner();
        emitConstructor();
        for(jsParser.StatementContext stm :ctx.block().statementList().statement()){
            if (stm.declarations()!=null) {
                emitSubroutines(stm.declarations().routinesPart());
            }
        }
        emitMainMethod(ctx);
    }
    ...
    private void emitProgramVariables()
    {
        ...
        for (SymtabEntry id : ids)
        {
            ...
            {
                System.out.println("emitProgramVariables() in ProgramGenerator: name: " + id.getName() + " value:" + id.getValue());
                if(id.getValue() instanceof Integer){
                    String value = id.getValue().toString();

                    emitDirectiveForValue(FIELD_PRIVATE_STATIC, id.getName(),
                            typeDescriptor(id),value);

                }else if(id.getValue() instanceof Float){
                        String value = id.getValue().toString();
     ...
     */
    private void emitSubroutines(jsParser.RoutinesPartContext ctx)
    {
        if (ctx != null)
        {
            for (jsParser.RoutineDefinitionContext defnCtx :
     ...
     * @param ctx the ProgramContext.
     */
    private void emitMainMethod(jsParser.ProgramContext ctx)
    {
        emitLine();
        emitComment("MAIN");
     ...
     */
    public void emitRoutine(jsParser.RoutineDefinitionContext ctx)
    {
        ...
        // Emit code for the compound statement.
        jsParser.BlockContext stmtCtx =
            (jsParser.BlockContext) routineId.getExecutable();
